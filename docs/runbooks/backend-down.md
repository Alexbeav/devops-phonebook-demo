...
- **Solution**: 
  ...
  docker pull ghcr.io/alexbeav/devops-phonebook-demo/backend:<tag>
  
  # Update image tag if needed
  kubectl set image deployment/myapp-backend backend=ghcr.io/alexbeav/devops-phonebook-demo/backend:<new-tag> -n <namespace>
  ```
